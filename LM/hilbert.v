...
From Coq Require Import Decidable FunctionalExtensionality ssreflect.
From LM Require Export logic_tricks R_compl linear_map.
...
(*fold (Rsqr (Hnorm (plus (minus u f) (minus u g)))). does not work anymore! *)
replace (Hnorm _ * Hnorm _) with (Rsqr (Hnorm (plus (minus u f) (minus u g)))); try easy.
f_equal.
...
Hypothesis phi_mod: compatible_ms phi.
...
exists zero.
apply (compatible_ms_zero phi phi_mod).
intros v v' Hv Hv'.
...
Variable phi:E -> Prop.
Hypothesis phi_compat: compatible_ms phi.
Hypothesis phi_compl: complete_subset phi.

Definition orth_compl := fun x:E => forall (y:E), phi y -> inner x y = 0.

Lemma orth_compl_compat: compatible_ms orth_compl.
Proof.
