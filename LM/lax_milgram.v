...
From Coq Require Export Decidable FunctionalExtensionality ProofIrrelevanceFacts Reals ssreflect.
From Coquelicot Require Export Coquelicot.
From LM Require Export logic_tricks R_compl compatible.
From LM Require Import hilbert.
From LM Require Export continuous_linear_map.
...
Lemma compatible_ms_ker : forall (f : E -> F), is_linear_mapping f ->
                  compatible_ms (ker f).
Proof.
intros.
unfold compatible_ms; unfold compatible_g.
split; intros.
split; intros.
unfold ker in *.
...
(* Unused.
Lemma compatible_ms_ker : forall (f : topo_dual E),
                  compatible_ms  (ker f).
Proof.
intros; apply compatible_ms_ker, Lf.
Qed.*)
...
Lemma clm_ker_convex2 : forall (f : topo_dual E),
                 convex (ker f).
Proof.
intros; apply clm_ker_convex, Lf.
Qed.*)
...
Lemma compatible_ms_and : forall (phi phi' : E -> Prop), compatible_ms phi ->
          compatible_ms phi' -> compatible_ms (fun x : E => phi x /\ phi' x).
Proof.
intros phi0 phi0' C C'.
unfold compatible_ms.
split.
unfold compatible_g.
split.
intros x y H H0.
...
apply compatible_ms_and.
apply compatible_ms_ker.
apply f.
trivial.
...
apply: compatible_ms_zero.
apply compatible_ms_ker, f.
destruct m_C as ((CG1,(z,CG2)),CS).
specialize (CS z 0%R CG2).
now rewrite scal_zero_l in CS.
split.
assert (compatible_ms (fun x : E => ker f x)).
apply compatible_ms_ker, f.
...
easy.
assert (Hmm : compatible_ms (fun x : E =>  True)).
